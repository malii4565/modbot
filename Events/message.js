...
if(!prefix.some(x => message.content.startsWith(x))) return
    ...
    if (!owners.includes(message.author.id)) {
        ...
        if (blockArr.length == 5) {
            message.channel.send(`${message.author}` + "```⛔ Komut kullanımın kötüye kullandığın için engellendi.Açtırmak için " + "' ✙ Mali#1611 veya ✙ Luke#1611" + " kişisine ulaşman gerekiyor...```")
    ...
    client.channels.cache.get("929107777337573407").send(`:zap: \`${message.author.tag}\` kişisi \`${message.channel.name}\` adlı kanalda \`${cmd.commandSettings.name}\` komutunu kullandı \n Komutun İçeriği => \`${message.content}\` `,{ignoreDirect: false})
